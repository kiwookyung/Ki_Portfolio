export const career = [
  {
    id: 1,
    type: "education",
    org: "조선대학교",
    title: "수학과",
    start: "2019-03",
    end: "2025-02",
    period: "2019.03 - 2025.02",
    location: "광주",
    description:
      "수학적 사고와 문제 해결력을 바탕으로 데이터 분석 및 알고리즘 이해를 확장.",
    details: [
      "해석학·선형대수·통계학 기초 이수",
      "모델링 및 논리적 문제 해결 역량 강화",
    ],
    logo: "/images/chosun-univ.png",
  },
  {
    id: 2,
    type: "education",
    org: "SSAFY (삼성 청년 SW 아카데미) 13기",
    title: "SW 개발자 과정 (진행 중)",
    start: "2025-01",
    end: null,
    period: "2025.01 - 현재",
    description: "SW 교육과정 및 프로젝트를 통해 실무 경험을 쌓고 있습니다.",
    details: [
      "1학기 수료, 2학기 진행 중",
      "ReeLoom · FloodGuard · OrakGarak 등 팀 프로젝트 수행",
    ],
    logo: "/images/ssafy.png",
  },
  {
    id: 3,
    type: "activity",
    org: "조선대학교 산업협력단",
    title: "디지털 새싹 캠프 - SW·AI 보조 강사",
    start: "2023-07",
    end: "2023-08",
    period: "2023.07 - 2023.08",
    description: "초·중등학생 대상 디지털 인재 양성 캠프에서 SW·AI 분야 보조 강사로 참여.",
    details: [
      "Python, 인공지능(TensorFlow, Teachable Machine), 메타버스 콘텐츠 제작 등 실습형 교육 과정 기획 및 지도",
      "학습자의 수준에 맞춘 콘텐츠 설명과 실습 지원, 결과물 피드백 제공",
      "현장 강의자료 제작 및 기술 시연을 통해 실전 교육 커뮤니케이션 능력 향상"
    ],
    logo: "/images/digitalsaesak.png"
  },
  {
    id: 4,
    type: "activity",
    org: "전남대학교 창업지원단",
    title: "호남·제주권 제품 제작 역량 강화 캠프 - 창업 아이디어 기획",
    start: "2024-01",
    end: "2024-01",
    period: "2024.01 - 2024.01",
    description: "지역 문제 해결을 위한 창업 아이디어 발굴 및 비즈니스 모델 기획 캠프 참가.",
    details: [
      "팀 프로젝트로 IT 기반 창업 모델 구체화 및 비즈니스 모델 캔버스 설계",
      "시장·경쟁사 분석 및 고객 타깃 정의를 기반으로 IR 발표 자료 제작",
      "창업 멘토링을 통한 피드백 반영 및 실현 가능성 중심 피칭 발표 수행"
    ],
    logo: "/images/startup.png"
  }  
];
